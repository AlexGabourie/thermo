...
def corr(f, g, max_lag):
    '''
    Computes fast correlation function and returns up to max_lag. Assumes f and g are same length.
    ...
    d = N - np.arange(N)
    f = np.lib.pad(f, (0,N), 'constant', constant_values=(0,0))
    g = np.lib.pad(g, (0,N), 'constant', constant_values=(0,0))
